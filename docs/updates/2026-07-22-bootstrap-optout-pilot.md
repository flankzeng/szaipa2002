...
`_newLayout.cshtml` 仍默认加载外接 Bootstrap 3 CSS；只有页面显式设置 `ViewData["UseBootstrapCss"] = false` 时，才在相同位置改载带内容版本的 `public-bootstrap-baseline.css`。当前只放行四个没有 Bootstrap 组件或标准 12 栏 class 的页面：
...
轻量基线保留这些页面和公共 navbar/footer 实际依赖的 Bootstrap/normalize 行为：`border-box`、body 字体/行高/颜色/背景、HTML5 block 元素、链接状态、图片/figure、标题字重/行高/margin、段落和列表 margin、NewAbout 文档按钮的字体/行高/外观归一化，以及这些页面相关的打印规则。文件保留 Bootstrap 3.3.7 与 normalize.css 的 MIT 归属说明。

其他页面继续走默认 Bootstrap，不受试点影响。特别是 NewIndex 仍有 `.clearfix` 依赖，不能直接全站删除；数据展览和特殊展览也继续保留原加载边界。

## 体积

旧发布版的 `bootstrap.css` 为 `143,947B`；轻量基线为 `2,564B`，每个试点页冷加载原始体积减少 `141,383B`。本机同参数对照：

- gzip -9：`20,871B → 1,092B`，减少 `19,779B`。
- Brotli quality 11：`16,989B → 832B`，减少 `16,157B`。
