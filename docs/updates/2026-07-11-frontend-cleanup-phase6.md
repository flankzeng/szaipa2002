...
- 临时只读导出检查了 Szaipa 19 个、Tongou 2 个资源相关字符串列，取得 19 条明确包含 `/Content/` 的路径，均位于 `images`。
- 加入数据库路径后重新审计，待复核仍为 1423 文件 / 95.9MB；`_preview`、`TempFile`、`Award`、根 `js`、`layui`、`Filme` 均无数据库精确引用，下一道证据只剩 IIS 访问日志。
