...
1. **Phase 5**：访问分析仪表盘（`AccessData` 省市聚合喂图表）+ Index 仪表盘 + OperationRecord / PasswordChange —— 中等，需选图表库。
2. **展览「参展作品目录」**：新增 `ExhibitionWork` 实体 + works 管理器（分类/标题/艺术家/尺寸/材质/图 + 排序）—— 中等偏新，重要型可选增强。
3. **slug 页退役**：把 ~16 个 `Views/Publication/*.cshtml` 迁成 `Publication` 行后删硬编码视图、首页改链 —— 机械。
4. **Phase 6 加固**：与 legacy 比对、anti-forgery/授权审、补单测。
...
约束：构建/测试用 ~/.dotnet/dotnet build|test Szaipa.Modernization.slnx（须 0/0 + 全绿，当前 69）；前端 cd src/Szaipa.Web && npm run build。DB 只读、绝不用生产凭据、不写 Windows 连的库；admin 写本地副本。没有可写库时只做 代码+单测+路由冒烟(302)，端到端留给用户本地。边做边验证、遇 legacy bug 顺手修。
