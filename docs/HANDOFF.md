...
- **Phase 3（部分）**：Fav / Auction / Exhibition（泛型基类 `ArtistScopedAdminRepository<T>`）、**Artist 主表 CRUD（2026-06-24 新增，照 News 范本而非泛型子类；详见 `docs/updates/2026-06-24-artist-admin-module.md`）**。
...
1. **Phase 3 收尾**：Works（图 + 尺寸 + 标签 + 富文本，`IArtistScopedRecord`，依赖已落地的 Artist 主表）、ArtWorks。镜像现有 admin 模块。
...
约束：构建/测试用 ~/.dotnet/dotnet build|test Szaipa.Modernization.slnx（须 0/0 + 全绿，当前 57）；前端 cd src/Szaipa.Web && npm run build。DB 只读、绝不用生产凭据、不写 Windows 连的库；admin 写本地副本。没有可写库时只做 代码+单测+路由冒烟(302)，端到端留给用户本地。边做边验证、遇 legacy bug 顺手修。
