...
- `_newLayout` / `_Artist` 默认仍加载 Bootstrap；NewIndex/NewVip/PublicationList/NewNews/NewAbout/NewArt/Publication 显式 `UseBootstrapCss=false`，在相同层级改载带版本的 `public-bootstrap-baseline.css`。七页没有 Bootstrap 组件/标准 12 栏，轻量基线保留公共布局、标题、表单、NewIndex `.clearfix`/36px `h1` 和 NewArt 导航 table 实际需要的 reset；其他页面不得未经逐页验证直接退出。Publication 于 2026-07-26 经 390/1440 严格 A/B 及 991/992 断点复核，几何与关键 computed style 一致。见 `docs/updates/2026-07-22-bootstrap-optout-pilot.md`。
...
## SQL 迁移
- `docs/sql/` 放显式 SQL（如 `2026-06-exhibition-template-columns.sql` 给 Publication 加 Type/Preface/Signature）。`2026-07-25-publication-migration-readiness.sql` 只读检查 schema/保留 ID；`2026-07-09-publication-slug-migration.sql` 为未来可选的事务化 fail-closed 归一化工具。14 个退役展览的公开渲染不依赖执行该写脚本。
