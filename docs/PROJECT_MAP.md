...
- `_newLayout` 默认仍加载 Bootstrap；NewVip/PublicationList/NewNews/NewAbout 显式 `UseBootstrapCss=false`，在相同层级改载带版本的 `public-bootstrap-baseline.css`。四页没有 Bootstrap 组件/标准栅格，轻量基线保留公共布局、新闻标题和 NewAbout 文档按钮实际需要的 reset；其他页面不得未经逐页验证直接退出。见 `docs/updates/2026-07-22-bootstrap-optout-pilot.md`。
