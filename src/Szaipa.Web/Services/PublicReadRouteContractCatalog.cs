...
public static class PublicReadRouteContractCatalog
{
        ...
        [
            ...
            new RoutePreviewGroupViewModel
            {
                ...
                [
                    new RoutePreviewLinkViewModel
                    {
                        Title = "Tongou artist skeleton",
                        Route = "/Project_Tongou/Atrist/1",
                        Status = "Dedicated skeleton with asset dependency panel"
                    },
                    new RoutePreviewLinkViewModel
                    {
                        Title = "Tongou special artist redirect target",
                        Route = "/Project_Tongou/beiniTD",
                        Status = "Dedicated skeleton preserving legacy id 96 redirect behavior"
                    },
                    new RoutePreviewLinkViewModel
                    {
                        Title = "Tongou work-list skeleton",
                        Route = "/Project_Tongou/WorkList/1",
                        Status = "Dedicated skeleton with asset dependency panel"
                    },
                    new RoutePreviewLinkViewModel
                    {
                        Title = "Tongou work skeleton",
                        Route = "/Project_Tongou/Work/1",
                        Status = "Dedicated skeleton with asset dependency panel"
                    },
                    new RoutePreviewLinkViewModel
                    {
                        Title = "Tongou special work redirect targets",
                        Route = "/Project_Tongou/beini",
                        Status = "Dedicated skeleton preserving legacy work-id redirect behavior"
                    }
                ]
            }
        ];
    ...
    public static IReadOnlyList<PublicReadRouteReadinessViewModel> CreatePublicReadRouteReadiness()
    {
        ...
        [
            ...
            CreateReadiness("Tongou", CreateTongouArtistDefinition(1)),
            CreateReadiness("Tongou", CreateTongouWorkListDefinition(1)),
            CreateReadiness("Tongou", CreateTongouWorkDefinition(1)),
            CreateReadiness("Tongou", CreateTongouSpecialArtistDefinition()),
            CreateReadiness("Tongou", CreateTongouSpecialWorkDefinition(
                "/Project_Tongou/beini · mouliqiao · mouliqiao2 · mengboshen · gaodaqing2",
                84,
                "Legacy special work ids 84/115/162/36/163 each resolve into a dedicated controller-owned route/view."))
        ];
    }
    ...
    public static PageSkeletonDefinition CreateTongouArtistDefinition(int id)
    {
        return new PageSkeletonDefinition
        {
            Title = "Tongou Artist Migration Skeleton",
            Eyebrow = "Tongou Browsing",
            Lead = "This Tongou artist page skeleton is ready for the first read-only single-artist flow. Legacy Atrist loads only the artist (no works) and links to WorkList for the works grid.",
            LegacyRoute = $"/Project_Tongou/Atrist/{id}",
            TargetRepository = "ITongouReadRepository.GetArtistByIdAsync",
            TargetBatch = "Tongou browsing",
            Sections =
            [
                new PageSkeletonSectionViewModel
                {
                    Title = "Artist header",
                    Description = "Read-only Tongou artist identity, title, header image, and about text."
                },
                new PageSkeletonSectionViewModel
                {
                    Title = "Works link",
                    Description = "Legacy Atrist renders only a link to /Project_Tongou/WorkList/{id}; the works grid lives on WorkList, not here."
                }
            ],
            AssetDependencies = TongouAssets,
            QueryParameters =
            [
                new PageSkeletonQueryParameterViewModel
                {
                    ...
                    Value = "1",
                    Reason = "The legacy route defaults to artist id 1 when no route value is supplied."
                }
            ],
            Guardrails = TongouGuardrails,
            TargetReadModels = TongouArtistTargetModels,
            RemovedWriteBehaviors = TongouRemovedWrites,
            Blueprint = CreateBlueprint("ITongouReadRepository", "GetArtistByIdAsync", "GetArtistByIdAsync"),
            NextSteps = TongouPageSkeletonNextSteps
        };
    }

    public static PageSkeletonDefinition CreateTongouWorkDefinition(int id)
    {
        return new PageSkeletonDefinition
        {
            Title = "Tongou Work Migration Skeleton",
            Eyebrow = "Tongou Browsing",
            Lead = "This Tongou work detail skeleton is reserved for the first read-only work page plus legacy redirect compatibility.",
            LegacyRoute = $"/Project_Tongou/Work/{id}",
            TargetRepository = "ITongouReadRepository.GetWorkByIdAsync",
            TargetBatch = "Tongou browsing",
            Sections =
            [
                new PageSkeletonSectionViewModel
                {
                    Title = "Work hero",
                    Description = "Primary work image, title, type, size, and creation date."
                },
                new PageSkeletonSectionViewModel
                {
                    Title = "Legacy redirect rules",
                    Description = "Special ids remain controller concerns while the repository stays read-only."
                }
            ],
            AssetDependencies = TongouAssets,
            QueryParameters =
            [
                new PageSkeletonQueryParameterViewModel
                {
                    Name = "WorkLookup",
                    Value = "Id",
                    Reason = "The legacy work detail flow starts from one TongouWorks row by primary key."
                }
            ],
            Guardrails = TongouGuardrails,
            TargetReadModels = TongouWorkTargetModels,
            RemovedWriteBehaviors = TongouRemovedWrites,
            Blueprint = CreateBlueprint("ITongouReadRepository", "GetWorkByIdAsync", "GetWorkByIdAsync"),
            NextSteps = TongouPageSkeletonNextSteps
        };
    }

    public static PageSkeletonDefinition CreateTongouWorkListDefinition(int id)
    {
        return new PageSkeletonDefinition
        {
            Title = "Tongou Work List Migration Skeleton",
            Eyebrow = "Tongou Browsing",
            Lead = "This Tongou work-list skeleton is ready for the first read-only artist-plus-works snapshot flow.",
            LegacyRoute = $"/Project_Tongou/WorkList/{id}",
            TargetRepository = "ITongouReadRepository.GetArtistProfileAsync",
            TargetBatch = "Tongou browsing",
            Sections =
            [
                new PageSkeletonSectionViewModel
                {
                    ...
                    Description = "Display the selected Tongou artist name and meta before the work list."
                },
                new PageSkeletonSectionViewModel
                {
                    Title = "Work grid",
                    Description = "Read-only list of Tongou works filtered by Atristid."
                }
            ],
            AssetDependencies = TongouAssets,
            QueryParameters =
            [
                new PageSkeletonQueryParameterViewModel
                {
                    Name = "DefaultArtistId",
                    Value = "1",
                    Reason = "The legacy work-list route defaults to artist id 1 when the route value is missing."
                },
                new PageSkeletonQueryParameterViewModel
                {
                    Name = "WorksFilter",
                    Value = "Atristid == id",
                    Reason = "The old page filters TongouWorks by Atristid before rendering the list."
                }
            ],
            Guardrails = TongouGuardrails,
            TargetReadModels = TongouArtistProfileTargetModels,
            RemovedWriteBehaviors = TongouRemovedWrites,
            Blueprint = CreateBlueprint("ITongouReadRepository", "GetArtistProfileAsync", "GetArtistProfileAsync"),
            NextSteps = TongouPageSkeletonNextSteps
        };
    }

    public static PageSkeletonDefinition CreateTongouSpecialArtistDefinition()
    {
        return new PageSkeletonDefinition
        {
            Title = "Tongou Special Artist Skeleton",
            Eyebrow = "Tongou Browsing",
            Lead = "This dedicated Tongou artist route preserves the old beiniTD redirect target before EF Core wiring begins.",
            LegacyRoute = "/Project_Tongou/beiniTD",
            TargetRepository = "ITongouReadRepository.GetArtistByIdAsync(id: 96)",
            TargetBatch = "Tongou browsing",
            Sections =
            [
                new PageSkeletonSectionViewModel
                {
                    Title = "Dedicated artist presentation",
                    Description = "Legacy artist id 96 uses its own endpoint/view and stays separate from generic artist rendering."
                },
                new PageSkeletonSectionViewModel
                {
                    Title = "Controller-owned compatibility",
                    Description = "Keep this redirect rule in the controller while the repository stays focused on read-only projection."
                }
            ],
            AssetDependencies = TongouAssets,
            QueryParameters =
            [
                new PageSkeletonQueryParameterViewModel
                {
                    Name = "DedicatedArtistId",
                    Value = "96",
                    Reason = "The legacy beiniTD target always resolves from artist id 96."
                }
            ],
            Guardrails = TongouGuardrails,
            TargetReadModels = TongouArtistTargetModels,
    ...
    public static PageSkeletonDefinition CreateTongouSpecialWorkDefinition(string legacyRoute, int workId, string compatibilityDetail)
    {
        return new PageSkeletonDefinition
        {
            Title = "Tongou Special Work Skeleton",
            Eyebrow = "Tongou Browsing",
            Lead = "This dedicated Tongou work route preserves a legacy redirect target before the read-only EF Core implementation is wired.",
            LegacyRoute = legacyRoute,
            TargetRepository = $"ITongouReadRepository.GetWorkByIdAsync(id: {workId})",
            TargetBatch = "Tongou browsing",
            Sections =
            [
                new PageSkeletonSectionViewModel
                {
                    Title = "Dedicated work presentation",
                    Description = $"Legacy work id {workId} resolves into its own endpoint/view instead of the generic work detail path."
                },
                new PageSkeletonSectionViewModel
                {
                    Title = "Controller-owned compatibility",
                    Description = compatibilityDetail
                }
            ],
            AssetDependencies = TongouAssets,
            QueryParameters =
            [
                new PageSkeletonQueryParameterViewModel
                {
                    Name = "DedicatedWorkId",
                    Value = workId.ToString(),
                    Reason = $"This legacy target always resolves from Tongou work id {workId}."
                }
            ],
            Guardrails = TongouGuardrails,
            TargetReadModels = TongouWorkTargetModels,
    ...
    private static readonly PageSkeletonGuardrailViewModel[] TongouGuardrails =
    [
        new()
        {
            Title = "No Tongou view-count writes",
            Detail = "Do not increment TongouWorks.VisityCount and keep SaveChanges out of all public Tongou requests."
        },
        new()
        {
            Title = "Redirect rules stay in controllers",
            Detail = "Legacy id-based route branching belongs in the controller layer so the repository stays focused on read-only projection."
        }
    ];

    private static readonly PageSkeletonTargetModelViewModel[] TongouArtistTargetModels =
    [
        new()
        {
            Name = "TongouArtistModel",
            Purpose = "Carries the dedicated Tongou artist identity, title, header image, and about text fields."
        }
    ];

    private static readonly PageSkeletonTargetModelViewModel[] TongouWorkTargetModels =
    [
        new()
        {
            Name = "TongouWorkModel",
            Purpose = "Carries the Tongou work detail fields such as title, image path, type, size, and creation date."
        }
    ];

    private static readonly PageSkeletonTargetModelViewModel[] TongouArtistProfileTargetModels =
    [
        new()
        {
            Name = "TongouArtistProfileSnapshotModel",
            Purpose = "Wraps one Tongou artist plus the linked works list for artist and work-list routes."
        },
        new()
        {
            Name = "TongouArtistModel",
            Purpose = "Carries the selected artist identity and descriptive fields."
        },
        new()
        {
            Name = "TongouWorkModel",
            Purpose = "Carries the work summaries shown under the selected Tongou artist."
        }
    ];

    private static readonly PageSkeletonRemovedWriteViewModel[] TongouRemovedWrites =
    [
        new()
        {
            LegacyBehavior = "TongouWorks.VisityCount++",
            Reason = "Public Tongou work requests must not change counters during the read-only migration."
        },
        new()
        {
            LegacyBehavior = "SaveChanges()",
            Reason = "Tongou public routes stay projection-only until a later, explicit write policy is introduced."
        }
    ];

    private static readonly string[] TongouPageSkeletonNextSteps =
    [
        "Generate the read-only EF Core context for Tongou.",
        "Implement the target repository method with AsNoTracking queries.",
        "Port the Razor view against the new read model.",
        "Verify output against the latest publish snapshot before enabling the route."
    ];
    ...
    private static readonly PageAssetDependencyViewModel[] TongouAssets =
    [
        new()
        {
            SourcePath = "<ContentRoot>/icon",
            TargetPath = "src/Szaipa.Web/wwwroot/legacy/content/icon",
            Reason = "Shared navigation accents reused by Tongou pages if the migrated views still depend on them."
        },
        new()
        {
            SourcePath = "<ContentRoot>/fonts",
            TargetPath = "src/Szaipa.Web/wwwroot/legacy/content/fonts",
            Reason = "Typography parity if Tongou views keep legacy font usage."
        }
    ];
}
